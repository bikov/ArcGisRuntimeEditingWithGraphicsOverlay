...
namespace SceneEditingDemo.Helpers
{
    ...
    public class SceneEditHelper
	{
        ...
        public static async Task<MapPoint> CreatePointAsync(MapView sceneView)
        {
            Initialize();
        ...
        public static async Task<Polyline> CreatePolylineAsync(MapView sceneView)
		{
            Initialize();
        ...
        public static async Task<Polygon> CreatePolygonAsync(MapView sceneView)
        {
            Initialize();
        ...
        public static async Task<Polygon> EditPolygonAsync(MapView sceneView, Polygon polygon)
		{
            Initialize();
        ...
        /// </exception>
        /// <returns>Return edited <see cref="Polygon"/> based on the user interactions.</returns>
        public static async Task<Polyline> EditPolylineAsync(MapView sceneView, Polyline polyline)
		{
            Initialize();
			...
			Action cleanupEvents = SetUpHandlers(sceneView,
				(p) => //On mouse move, move completion line around
				{
					if (p != null && isEditingVertex)
					{
						...
						var index = vertexPoints
							.IndexOf(vertexPoints.First(point => GeometryEngine.Equals(point, selectedVertex.Geometry)));
						var temporaryVertices = new List<MapPoint>();
		...
		private static Action SetUpHandlers(MapView view, Action<MapPoint> onMove, Action<MapPoint> onTapped, Action<MapPoint> onDoubleTapped)
		{
#if NETFX_CORE
			...
			if (onTapped != null)
			{
				...
				view.MapViewTapped += tappedHandler;
			}
			...
			if (onDoubleTapped != null)
			{
				...
				view.MapViewDoubleTapped += doubletappedHandler;
			}
			Action cleanup = () =>
			{
				if (movehandler != null)
					...
					view.MouseMove -= movehandler;
#endif
				if (tappedHandler != null) view.MapViewTapped -= tappedHandler;
				if (doubletappedHandler != null) view.MapViewDoubleTapped -= doubletappedHandler;
			};
			return cleanup;
		}
